...
def highlight_priority(row):
    try:
        ...
        pr = 0
    if pr == 1:
        return ["background-color: #ffe6e6; font-weight:700;" for _ in row.index]
...
def main_dashboard():
    ...
    if page == "Overview":
        ...
        for i, s in enumerate(STATUS_OPTIONS):
            with cols[i]:
                st.markdown(f'<div style="padding:14px;border-radius:8px;background:{colors[i]};box-shadow:0 2px 6px rgba(0,0,0,0.08)">' + 
                            f'<h3 style="margin:0;color:#0b3b5c">{s}</h3>' +
        ...
        with st.expander("Filters & Search", expanded=True):
            ...
            q_search = st.text_input("Search server, registration, comments")
        ...
        if q_search:
            mask = (filtered['server'].str.contains(q_search, na=False, case=False)) | \
                   (filtered['registration'].str.contains(q_search, na=False, case=False)) | \
                   (filtered['comments'].str.contains(q_search, na=False, case=False))
        ...
        filtered = filtered.sort_values(by='priority')
        ...
        if st.session_state.role == 'admin':
            ...
            with st.form("add_record", clear_on_submit=True):
                ...
                if st.form_submit_button("Add record"):
                    new['date_created'] = str(new['date_created'])
